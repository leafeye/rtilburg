...
Welcome to the course 'Research Skills: Programming with R'! You're looking at Worksheet 1. It contains both informative text and exercises, and if you work through each week's material, you should be sufficiently prepared for the assignments and group project that make up the final examination.
...
my_new_obj <- "orange"
#this will create a new object, with the string "orange" attached to it

5_is_a_number <- TRUE
#this will give an error? it starts with a number and this is not allowed
```
...
first_object <- 2
second_object <- 3
third_object = first_object + second_object
...
round(3.65, "1")
#it gives an error because the second argument is a string
...
rnorm(n = 1, mean = 0, sd = 3) ## draw 1 sample

## extract the letters 'red' from 'credit' using 'substr()'
substr("credit", start = 2, stop = 4)
```
...
as.character(round(3.67, digits = 1))
#the first one doesnt include the digits = 1 for the round function
```
...
newbird = paste0(toupper(substr(bird_name, start = 1, stop = 1)), substr(bird_name, start = 2, stop = 5))


paste(newbird, ". Weight:", bird_weight, weight_unit, ". Wingspan:", 
  bird_span, span_unit, ".", sep = "")
...
## answer: concatenating the two 'mix_vecs'
mix_vec_3 <- c(mix_vec_1, mix_vec_2)

## why is this vector different?
all_at_once_vec <- c("apple", 1.6, TRUE, 5, 4.9, FALSE)
#the False value is not converted to a zero
...
zoolist <- list(name = "Artis", animals = c("lion", "elephant", "rhino", "pinguin"), profit = 20000)
...
animals <- c("lion", "elephant", "rhino", "pinguin")
amount <- c(3, 2, 2, 8)
avgweight <- c(150, 300, 280, 20)
avgheight <- c(110, 230, 180, 50)

animaldf <- data.frame(animals, amount, avgweight, avgheight)
animaldf
...
my_char_vec[-1]
my_char_vec[c(-1, -3)]
#negative indices returns everything but the index nr
...
round(my_list[2])
#it is the second element of the list, but not the value it contains
...
You were probably expecting a vector of characters as the result of this operation. But there are no quotation marks around the individual items, so they are not characters, and there's this strange line about `levels` ...?
...
We'll discuss factors in more detail in Worksheets 3 & 6, but for now, just remember that once's something is a factor, you can't just add new values to it; unless the new value is one of the existing `levels`, you'll get a `<NA>` instead, where `NA` is a special R value indicating 'not available':
...
Better yet, of course, is to avoid the conversion to `factor()` in the first place; always set `stringsAsFactors = FALSE` when building a new data frame.
...
my_df$goodness <- (my_df$tastyness + my_df$healthyness) / 2
my_df
...
testdf <- data.frame(one = c(1:6))
testdf$two <-runif(6, 1, 10)
testdf$three <- testdf$one * testdf$two
testdf$four <- c(testdf$one[1:3], testdf$two[1:3])
testdf$five <- testdf$one < testdf$two
testdf$six <- c("12","1edag", "_score", "dag,uur", "naam:", "/score")
testdf
